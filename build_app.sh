#!/bin/zsh
# Самодостаточный Dubl.app: свой Python, библиотеки, ffmpeg и yt-dlp.
# Веса моделей качаются из настроек — иначе бандл на сорок гигабайт.
set -e
cd "${0:A:h}"
SRC="$PWD"
OUT="$SRC/build"
...
"$PY" -m pip -q install PySide6 mlx-lm mlx-whisper sherpa-onnx soundfile scipy numpy \
    yt-dlp num2words demucs torch torchaudio pyworld mlx-audio
# torch нужен demucs для отделения голоса, поэтому остаётся целиком.

say "чистка лишнего"
SP="$RES/python/lib/python3.12/site-packages"
# Лишние модули Qt. Веб-движок нужен под встроенный плеер ссылок,
# QtMultimedia — под проигрыватель файлов и готового дубляжа.
for m in Qt3D QtCharts QtDataVis QtDesigner QtQuick3D QtSensors QtSerial QtSql \
         ...
         QtStateMachine QtTextToSpeech QtHelp QtUiTools QtPdf; do
...
cp app.py pipeline.py source.py _style.py models_store.py i18n.py "$RES/app/"
mkdir -p "$RES/app/models"
./fetch_models.sh
cp -R models/nemo_en_titanet_large.onnx \
      models/sherpa-onnx-pyannote-segmentation-3-0 "$RES/app/models/"
...
Из чего собран Dubl и на каких условиях.
...
Qwen3 и Qwen3-TTS — Apache 2.0, Whisper — MIT, HTDemucs — MIT.
LICNOTE
...
#!/bin/zsh
LOG="$HOME/Library/Logs/Dubl.log"
...
printf '"CFBundleDisplayName" = "Dubl";\n"CFBundleName" = "Dubl";\n' \
...
<key>CFBundleDisplayName</key><string>Dubl</string>
<key>CFBundleExecutable</key><string>launcher</string>
<key>CFBundleIconFile</key><string>AppIcon</string>
<key>CFBundleIdentifier</key><string>com.local.dubl</string>
<key>CFBundleInfoDictionaryVersion</key><string>6.0</string>
<key>CFBundleName</key><string>Dubl</string>
<key>CFBundlePackageType</key><string>APPL</string>
<key>LSMinimumSystemVersion</key><string>13.0</string>
<key>CFBundleLocalizations</key><array><string>ru</string><string>en</string></array>
<key>NSHighResolutionCapable</key><true/>
<key>NSDocumentsFolderUsageDescription</key><string>Dubl сохраняет готовые конспекты в папку «Документы».</string>
<key>NSDownloadsFolderUsageDescription</key><string>Dubl может сохранять конспекты в папку «Загрузки».</string>
<key>NSDesktopFolderUsageDescription</key><string>Dubl может сохранять конспекты на рабочий стол.</string>
